...
- `Ctrl+Insert` / `Shift+Insert` / `Shift+Delete` – send standard terminal copy, paste, and cut keys. Neovim handles them mode-aware, including paste into terminal buffers from terminal-normal and visual modes; in terminal buffers, cut copies the output without trying to modify scrollback. tmux uses `Ctrl+Insert` in copy mode, passes copy/cut through to pane-aware apps, and uses `Shift+Insert` to paste into plain panes.
