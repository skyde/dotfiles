...
- `Ctrl+Insert` / `Shift+Insert` / `Shift+Delete` – send standard terminal copy, paste, and cut keys. Neovim handles them mode-aware, including terminal-mode copy/cut of the visible terminal line and paste into terminal buffers from terminal-normal, terminal-visual, and terminal-mode; terminal cut copies output without trying to modify scrollback. tmux uses `Ctrl+Insert` in copy mode, passes copy/cut through to pane-aware apps, and uses `Shift+Insert` to paste into plain panes.
