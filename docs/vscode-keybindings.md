...
- `Ctrl+Insert` / `Shift+Insert` / `Shift+Delete` – send standard terminal copy, paste, and cut keys. Neovim handles them mode-aware, including paste into terminal buffers from terminal-normal mode; tmux uses `Ctrl+Insert` in copy mode, passes copy/cut through to pane-aware apps, and uses `Shift+Insert` to paste into plain panes.
