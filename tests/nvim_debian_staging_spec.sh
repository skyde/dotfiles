...
require_line "$suite" 'rsync -a --inplace -- "$source_dir/" "$destination_dir/"'
require_line "$suite" 'sync -f "$destination_dir"'
require_line "$suite" 'checkpoint_marker_helper="$source_repo_root/tests/nvim-debian/checkpoint_marker.py"'
require_line "$suite" 'python3 "$checkpoint_marker_helper" invalidate'
require_line "$suite" 'publish_durable_checkpoint_marker "$marker"'
require_line "$suite" 'python3 "$checkpoint_marker_helper" retire'
require_line "$suite" 'python3 "$checkpoint_marker_helper" atomic-publish'
require_line "$suite" 'sync_artifacts || status=$?'
require_line "$suite" 'if sync_artifacts'
...
require_line "$runner" 'artifact_finalize_grace_seconds=${ARTIFACT_FINALIZE_GRACE_SECONDS:-400}'
require_line "$runner" 'minimum_finalize_grace=$((10 * (artifact_sync_timeout_seconds + timeout_kill_after_seconds) + 30))'
...
require_line "$suite" 'retire_durable_checkpoint_marker'
require_line "$suite" '&& publish_durable_marker "$marker" final-export-complete.env'
...
reject_line "$suite" '"$destination_dir/.checkpoint-complete.env.tmp"'
reject_line "$suite" 'mv -f -- "$2/.checkpoint-complete.env.tmp"'
