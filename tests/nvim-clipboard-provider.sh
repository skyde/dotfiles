...
cleanup() {
  ...
  rm -rf "$tmp"
  rm -f "$lua_file" "$runtime_lua_file" "$multiline_runtime_lua_file" "$empty_runtime_lua_file"
}
trap cleanup EXIT
...
if expected_copy_kind == "" then
  expected_copy_kind = expected_provider_name == "osc-copy/osc-paste" and "helper" or "any"
end
if expected_paste_kind == "" then
  expected_paste_kind = expected_provider_name == "osc-copy/osc-paste" and "helper" or "any"
end
...
if expected_copy_kind == "helper" or expected_paste_kind == "helper" then
...
run_provider_case copy-helper-only "$tmp/path-home" "$tmp/copy-only-bin" "$tmp/copy-only-bin:$tmp/tmux-bin:$system_path" 1 0 "" "" "" 0 0 0 "osc-copy/OSC 52" 0 "helper" "function"
run_provider_case paste-helper-only "$tmp/path-home" "$tmp/paste-only-bin" "$tmp/paste-only-bin:$tmp/tmux-bin:$system_path" 1 0 "" "" "" 0 0 0 "OSC 52/osc-paste" 0 "function" "helper"
