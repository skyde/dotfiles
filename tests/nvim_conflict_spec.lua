...
--------------------------------------------------------------------------
-- setup(): the autocmds that make highlighting happen on its own
--------------------------------------------------------------------------

do
  -- Scanning every buffer on every keystroke would mean reading the whole
  -- file per character typed, which is the cost this config exists to avoid.
  -- So it scans on read and on write, and only keeps watching the buffers
  -- that turned out to be conflicted. That balance is the thing to check.
  conflict.setup()
  local ns = vim.api.nvim_get_namespaces()["vcs_conflict"]
  ...
  local clean = write("clean.txt", { "nothing", "to", "see" })
  vim.cmd("edit " .. vim.fn.fnameescape(clean))
  eq("setup: a clean file gets no marks", 0, #vim.api.nvim_buf_get_extmarks(0, ns, 0, -1, {}))
  eq("setup: and is not watched", nil, vim.b.vcs_conflict_watched)

  local conflicted = write("conflicted.txt", zdiff3)
  vim.cmd("edit " .. vim.fn.fnameescape(conflicted))
  local buf = vim.api.nvim_get_current_buf()
  check("setup: opening a conflicted file highlights it", #vim.api.nvim_buf_get_extmarks(buf, ns, 0, -1, {}) > 0)
  eq("setup: and marks it watched", true, vim.b[buf].vcs_conflict_watched)

  -- Once watched, edits keep the colours honest.
  conflict.choose("ours")
  vim.api.nvim_exec_autocmds("TextChanged", { buffer = buf })
  local left = #conflict.list(buf)
  eq("setup: one conflict left after resolving the first", 1, left)
  check("setup: still highlighted", #vim.api.nvim_buf_get_extmarks(buf, ns, 0, -1, {}) > 0)

  conflict.choose_all("ours")
  vim.api.nvim_exec_autocmds("TextChanged", { buffer = buf })
  eq("setup: the marks clear once nothing is conflicted", 0, #vim.api.nvim_buf_get_extmarks(buf, ns, 0, -1, {}))

  -- A file that becomes conflicted later — a merge landing under an open
  -- buffer — is picked up on the next read.
  vim.fn.writefile(zdiff3, clean)
  vim.cmd("edit!")
  check("setup: a file that turns conflicted is picked up on reload", #conflict.list(0) > 0)

  -- Scratch buffers are skipped: they have no file to be conflicted about,
  -- and the diff view is full of them.
  vim.cmd("enew")
  vim.bo.buftype = "nofile"
  vim.api.nvim_buf_set_lines(0, 0, -1, false, zdiff3)
  vim.api.nvim_exec_autocmds("BufReadPost", { buffer = vim.api.nvim_get_current_buf() })
  eq("setup: nofile buffers are left alone", nil, vim.b.vcs_conflict_watched)
