...
# Adapted for use with stow-based dotfiles setup
...
install_from_github() {
  ...
  # Candidate URLs; try to download each until one succeeds
  local candidates=(
...
echo "Yazi installation script starting..."
      ...
      if HAVE apt-get; then
        ...
        if ! HAVE yazi; then
          if confirm_yazi_install "GitHub prebuilt binary (Debian/Ubuntu, GNU)"; then
            ...
            echo "[skip] yazi install (user declined)"
          fi
        else
          echo "[info] yazi is already installed"
        fi
        # Provide a 'bat' shim for tools that call 'bat' on Debian
        if ! HAVE bat && HAVE batcat; then
          target="$(command -v batcat)"
          if [ -n "$target" ]; then
            $SUDO_CMD install -d -m 0755 /usr/local/bin
            $SUDO_CMD ln -sf "$target" /usr/local/bin/bat
          fi
        fi
      fi
    else
      echo "[info] Custom Yazi install only supported on Debian/Ubuntu. Use your package manager instead."
    fi
    ;;
  Darwin)
    ...
    ;;
  *)
    echo "[info] Unsupported OS for automatic Yazi install: $OS"
    echo "[info] Please install yazi manually from: https://yazi-rs.github.io/docs/installation"
    ;;
esac
