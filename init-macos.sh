...
if [ -d "$HOME/Library/Fonts" ]; then
    jetbrains_nerd_font_found=0
    for font_path in "$HOME/Library/Fonts"/JetBrainsMono*Nerd*Font*.ttf "$HOME/Library/Fonts"/JetBrainsMono*Nerd*Font*.otf; do
        [ -e "$font_path" ] || continue
        jetbrains_nerd_font_found=1
        break
    done
    if [ "$jetbrains_nerd_font_found" -eq 0 ]; then
