...
AUTO_INSTALL=$(_truthy "${AUTO_INSTALL:-}") && AUTO_INSTALL=1 || AUTO_INSTALL=0
...
# Core policy:
# - present + AUTO_INSTALL=1 → proceed without asking
# - present + AUTO_INSTALL=0 → ask
# - missing (regardless of AUTO_INSTALL) → ask
confirm_change() {
  local verb="$1" name="$2" present="$3" # present: 1 or 0
  if [[ "$present" -eq 1 && "$AUTO_INSTALL" -eq 1 ]]; then
    return 0
  fi
  ask "$verb $name?"
}

# Generic ensure: pass a check, an install, and an update/configure action.
ensure() {
  local name="$1"; shift
  local check="$1"; shift
  local install="$1"; shift
  local update="${1:-}";        # optional
  local present=0
...
ensure_apt() {
  local pkg="$1"
    ...
    "sudo apt-get install -y --only-upgrade '$pkg' || true"
}
