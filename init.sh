...
if [ -f "packages.txt" ]; then
  ...
  if [[ "$(uname)" == "Linux" ]]; then
    for index in "${!packages[@]}"; do
      case "${packages[$index]}" in
        fd) packages[index]="fd-find" ;;
        delta) packages[index]="git-delta" ;;
      esac
    done
  fi
...
# Build bat's cache when bat is installed. AUTO_INSTALL=0 intentionally skips
# package installation, so a missing optional tool must not fail the bootstrap.
